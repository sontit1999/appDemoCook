...
object BindingUtils {
    ...
    @JvmStatic
    fun ImageView.setImageResoucre(res: Int) {
        if (res > 0)
    ...
    @JvmStatic
    fun ImageView.setImageResoucre(res: String) {
    ...
    fun setHeight(
        ...
        imageCollapsing: Int?,
        listImageSlide: MutableList<ItemImageSlide>?
    ) {
        val resourceId =
            collapsing.context.resources.getIdentifier("status_bar_height", "dimen", "android")
        val params = collapsing.layoutParams
        val styledAttributes =
            collapsing.context.theme.obtainStyledAttributes(intArrayOf(android.R.attr.actionBarSize))
    ...
    @JvmStatic
    fun setVisibility(view: View, isVisibility: Any?) {
        Log.d("", "Any ${isVisibility}")

        view.visibility = if (isVisibility == null) View.GONE else View.VISIBLE
    }

    @BindingAdapter("app:fitSystem")
    @JvmStatic
    fun setFitSystem(view: View, imageCollapsing: Int?) {
        view.fitsSystemWindows = imageCollapsing != null
    }

    @BindingAdapter("setImage")
    @JvmStatic
    fun setImage(view: ImageView, res: Any?) {
        if (res == null) return
    ...
    @JvmStatic
    fun setImageSliderFitSystem(view: ImageSlider, sliders: MutableList<ItemImageSlide>?) {
        view.fitsSystemWindows = sliders != null
    }

    @BindingAdapter("app:setViewPagerAdapter")
    @JvmStatic
    fun setAdapter(view: ImageSlider, sliders: MutableList<ItemImageSlide>?) {
    ...
    @JvmStatic
    fun setBackground(view: Toolbar, background: Int?) {
